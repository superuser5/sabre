...
class MultiShell( cmd.Cmd ) :
	...
	def __add_handler_shell( self, shell ) :
		print("Adding handler shell-")
		print(shell)
		#print(shell.handler.getOrchestrator().getIdentity()) #Error in this is that getOrchestrator is going to be the same
		session_id = input("Input the name of this new session: ")
		
		#OLD CODE
		#self.shells[shell.handler.getOrchestrator().getIdentity()] = shell
		
		#YOUNG CODE - Able to add in new sessions now 
		self.shells[session_id] = shell


	def list_sessions( self, verbose = False ) :

		handlers = [shell.handler for shell in list(self.shells.values())]
		#print("In list_sessions function - handlers is %s" % (handlers))
		to_show = []
		#print("self.shells is:")
		#print(self.shells)
	...
	def default(self, line) :
		...
		if not args.SESSIONS :
			...
			if resp.lower() != 'y' :
				...
				args.SESSIONS = list(self.shells.keys())

		for session_id in args.SESSIONS :
			if session_id in list(self.shells.keys()) :
	...
	def do_session( self, line ) :
		...
		if args.session_id in list(self.shells.keys()) :
	...
	def do_handler( self, line ) :
		...
		try:
			...
			print("HANDLER ADD or HANDLER DEL is required")
		


	def unmount_handler( self, orch_id, kill = False ) :
		if orch_id in list(self.shells.keys()) :
			if kill :
	...
	def mount_new_handler( self, filename, arguments, shell_var = 'shell' ) :	#  handler add examples/tcp_reverse_handler.py 8080 Pa55phra531
		if(isinstance(arguments, str)): #if passphrase is hardcoded
			
			sys.argv = arguments
		else:
			variables = ['handler_filename.py'] + arguments #adding in placeholder for handler_filename due to arguments only looking at index 1
			sys.argv = variables
		try:
			with open(filename, 'r') as handler_codefile :
				...
				try:
					...
					#print("HELLO WORLD")
					self.__add_handler_shell( namespace_dict[shell_var] ) #Using a dictionairy to store handler instances by specific shell locations in memory
					print("Added Session!")
				except IndexError:
					print("Error: You might have tried to add a handler that needed a passphrase to access")
